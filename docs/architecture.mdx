...
**Every engine is counted as one pool.** Ollama, LM Studio, vLLM, and SGLang load
is summed, and maximum GPU pressure applies to the whole node. That is conservative on a
...
known install locations for each engine, so an Ollama, LM Studio, vLLM, or SGLang
you installed yourself is found where it already is. "Installing" an engine that is already
