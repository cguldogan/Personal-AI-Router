...
**Put the PIN in `NVPAIR_PIN`, not on the command line, in a script.** A command
argument is visible to every user on the machine in `ps` and is written to your
shell history. The environment variable is neither.
...
2. On the box, over SSH:
   ...
   It exits `0` once the machines are paired.
...
ssh box-b "NVPAIR_PIN=$PIN nvpair-tui accept --wait 2m"
