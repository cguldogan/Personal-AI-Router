...
func TestNodeCandidates_OverlayOnlyNodeIsRoutable(t *testing.T) {
	tests := []struct {
		...
		{
			name: "cgnat only",
			node: Node{Addresses: []string{"100.101.102.103"}, Port: 1234},
			want: []string{"100.101.102.103:1234"},
		},
		{
			name: "ipv6 ula only",
			node: Node{Addresses: []string{"fd7a:115c:a1e0::1701:b2c3"}, Port: 1234},
			want: []string{"[fd7a:115c:a1e0::1701:b2c3]:1234"},
		},
		{
			name: "magicdns name only",
			node: Node{Addresses: []string{"gpu-box.tail1234.ts.net"}, Port: 1234},
			want: []string{"gpu-box.tail1234.ts.net:1234"},
		},
		{
		...
		},
		{
			...
			node: Node{Addresses: []string{"100.101.102.103", "192.0.2.10"}, Port: 1234},
			want: []string{"192.0.2.10:1234", "100.101.102.103:1234"},
		},
	}
...
func TestIsLocalAddress_CountsThisHostsOverlayAddresses(t *testing.T) {
	...
	got := nodeCandidates(Node{Addresses: []string{ourTailnetIP}, Port: 1234})
