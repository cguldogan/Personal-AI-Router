...
An SGLang model id is its `--served-model-name`, which defaults to `--model-path` verbatim — often a bare local directory such as `/models/my-model` rather than a Hugging Face id. It is reported as given and never parsed.
